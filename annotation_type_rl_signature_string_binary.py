#!/usr/bin/env python3
"""
Annotation Type Reinforcement Learning Training Script for @BinaryName
Uses binary RL models to train a separate model for predicting @BinaryName annotation placement
...
class AnnotationTypeTrainer:
    """Trainer for @BinaryName annotation type using Signature String Checker features"""
    
    def __init__(self, annotation_type='@BinaryName', base_model_type='gcn', learning_rate=0.001, device='cuda'):
    ...
    def _init_annotation_model(self):
        """Initialize model for @BinaryName annotation type prediction"""
    ...
    def extract_annotation_features(self, cfg_data, binary_predictions):
        ...
        for i, node in enumerate(nodes):
            ...
            # Determine if this node should have @BinaryName annotation
    ...
    def _should_have_annotation_type(self, node):
        """Determine if node should have @BinaryName annotation"""
        label = node.get('label', '').lower()
        
        # @BinaryName: for strings in slashed format (e.g., "java/lang/String")
        # Indicators:
        # - Has slashes (package separator)
        # - Used in Class.forName with slashed format
        # - Used in reflection APIs
        
        has_slashes = '/' in label
        is_forname_slashed = 'class.forname' in label and '/' in label
        is_reflection_api = any(pattern in label for pattern in ['class.getclass', 'class.getdeclaredmethod', 'method.invoke'])
        
        return has_slashes or is_forname_slashed or is_reflection_api
...
def main():
    parser = argparse.ArgumentParser(description=f'Training for @BinaryName annotation type')
    ...
    logger.info("@BinaryName annotation type training completed successfully")
