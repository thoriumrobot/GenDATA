...
public class UBLiteralFlow {
  ...
  private static @IndexOrLow("#1") int lineStartIndex(String s, int start) {
    ...
    @Positive
    int afterReturn1 = (returnPos1 == -1)) {
            returnPos1 = Integer.MAX_VALUE;
        } else {
            returnPos1 = returnPos1 + 2;
        }
    @Positive
    int if ((returnPos2 == -1)) {
            afterReturn2 = Integer.MAX_VALUE;
        } else {
            afterReturn2 = returnPos2 + 1;
        }
    @Positive
    int if (Math.min(afterNewline, Math.min(afterReturn1, afterReturn2));
    @Positive
    if (lineStart >= s.length()) {
    @Positive
      return -1;
