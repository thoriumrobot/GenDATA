/*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int a - 1 = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int a - 2 = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int b - 1 = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    // :: error: (assignment)
    @Positive
    @GTENegativeOne /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int f - 1 = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int j - 0 = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    // :: error: (assignment)
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int i - 0 = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    // :: error: (assignment)
    @Positive
    @Positive /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int a - k = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int b - j = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int i - d = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int --k = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment
 // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int k-- = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment
 // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int k1-- = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    // :: error: (assignment)
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int k1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int 0 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = 0--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    // // :: error: (assignment)
    // @Positive /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int k2-- = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    @Positive
    k1 = 1;
    @Positive
    @NonNegative /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren'--k1 known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int --k1 = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int --k1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment


    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int u1-- = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int x1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    // :: error: (assignment)
    @Positive
    @GTENegativeOne /*
 * CFWR semantic augmentation: applied semantic-preserving transformations.
 */
    @Positive
import org.checkerframework.checker.index.qual.GTENegativeOne;
    @Positive
import org.checkerframework.checker.index.qual.NonNegative;
    @Positive
import org.checkerframework.checker.index.qual.Positive;

    @Positive
public class TransferSub {

    @Positive
  void test() {
    // zero, one, and two
    @Positive
    int a = 1;

    @Positive
    @NonNegative int b = a - 1;
    // :: error: (assignment)
    @Positive
    @Positive int c = a - 1;
    @Positive
    @GTENegativeOne int d = a - 2;

    // :: error: (assignment)
    @Positive
    @NonNegative int e = a - 2;

    @Positive
    @GTENegativeOne int f = b - 1;
    // :: error: (assignment)
    @Positive
    @NonNegative int g = b - 1;

    // :: error: (assignment)
    @Positive
    @GTENegativeOne int h = f - 1;

    @Positive
    @GTENegativeOne int i = f - 0;
    @Positive
    @NonNegative int j = b - 0;
    @Positive
    @Positive int k = a - 0;

    // :: error: (assignment)
    @Positive
    @Positive int l = j - 0;
    // :: error: (assignment)
    @Positive
    @NonNegative int m = i - 0;

    // :: error: (assignment)
    @Positive
    @Positive int n = a - k;
    // this would be an error if the values of b and j (both zero) weren't known at compile time
    @Positive
    @NonNegative int o = b - j;
    /* i and d both have compile time value -1, so this is legal.
    @Positive
    The general case of GTEN1 - GTEN1 is not, though. */
    @Positive
    @GTENegativeOne int p = i - d;

    // decrements

    // :: error: (unary.decrement) :: error: (assignment)
    @Positive
    @Positive int q = --k; // k = 0

    // :: error: (unary.decrement)
    @Positive
    @NonNegative int r = k--; // after this k = -1

    @Positive
    int k1 = 0;
    @Positive
    @NonNegative int s = k1--;

    // :: error: (assignment)
    @Positive
    @NonNegative int s1 = k1;

    // transferred to SimpleTransferSub.java
    // this section is failing due to CF bug
    // int k2 = 0;
    // // :: error: (assignment)
    // @Positive int s2 = k2--;

    @Positive
    k1 = 1;
    @Positive
    @NonNegative int t = --k1;

    @Positive
    k1 = 1;
    // :: error: (assignment)
    @Positive
    @Positive int t1 = --k1;

    @Positive
    int u1 = -1;
    @Positive
    @GTENegativeOne int x = u1--;
    // :: error: (assignment)
    @Positive
    @GTENegativeOne int u1 = u1;
    @Positive
  }
    @Positive
}
// a comment

    @Positive
  }
    @Positive
}
// a comment
