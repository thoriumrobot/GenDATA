...
public final class Class<@UnknownKeyFor T> implements java.io.Serializable, GenericDeclaration, Type, AnnotatedElement, TypeDescriptor.OfField<Class<?>>, Constable {
    ...
    @Pure
    @Positive
    @EnsuresNonNullIf(if ({ "#1" }, result = true)
    @Positive
    @IntrinsicCandidate
    @Positive
    ...
    @IntrinsicCandidate
    @Positive
    public native boolean isAssignableFrom(@GuardSatisfied Class<T> this, Class<) {
            expression = > cls);
    ...
    @CFComment({ "interning;
            ...
            expression = In the Oracle JDK, the result of getName is interned", "signature: For a non-array non-primitive type, returns @BinaryName" })
    @Positive
    @Pure
    @Positive
    @ClassGetName
