#!/usr/bin/env python3
"""
Train All SQL Quotes Checker Models Using Balanced Datasets

This script trains all models for SQL Quotes Checker annotations using balanced datasets:
...
class BalancedSqlQuotesModelsTrainer:
    def __init__(self, cfg_dir=None, balanced_dataset_dir=None, models_dir=None, 
        ...
        """
        Initialize balanced SQL Quotes models trainer
        
        Args:
            cfg_dir: Directory containing CFG files (default: cfg_output_adaptive_specimin_sql_quotes/)
        ...
        """
        self.cfg_dir = cfg_dir or str(GEN_DATA_ROOT / 'cfg_output_adaptive_specimin_sql_quotes')
        self.balanced_dataset_dir = balanced_dataset_dir or str(GEN_DATA_ROOT / 'balanced_datasets_sql_quotes')
        self.models_dir = models_dir or str(GEN_DATA_ROOT / 'models_annotation_types_sql_quotes')
    ...
    def generate_balanced_datasets(self):
        """Generate balanced datasets for SQL Quotes Checker"""
        logger.info("=" * 80)
        logger.info("Generating Balanced Datasets for SQL Quotes Checker")
        logger.info("=" * 80)
        
        if not os.path.exists(self.cfg_dir):
            logger.error(f"CFG directory not found: {self.cfg_dir}")
            logger.error("Please run create_training_datasets.py --checker sql_quotes first")
            return False
    ...
    def train_all_models(self):
        """Train all SQL Quotes models using balanced datasets"""
        logger.info("=" * 80)
        logger.info("Training All SQL Quotes Models with Balanced Datasets")
        logger.info("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train SQL Quotes models using balanced datasets')
    ...
    trainer = BalancedSqlQuotesModelsTrainer(
