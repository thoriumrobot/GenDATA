...
class AnnotationTypeTrainer:
    ...
    def __init__(self, annotation_type='@GTENegativeOne', base_model_type='gcn', learning_rate=0.001, device='cuda', checker_type=None):
    ...
    def extract_annotation_features(self, cfg_data, binary_predictions):
        ...
        use_all_nodes = not binary_predictions  # fallback when no binary preds provided
    ...
    def _extract_annotation_type_features(self, node, cfg_data):
        ...
        # "Could be zero" detection patterns (focus on >= -1 patterns)
        ...
        # Pattern 7: Used in >= -1 check (explicit >= -1 check)
        used_in_gtenegativeone_check = any(pattern in label_lower for pattern in ['>= -1', '>=-1', '>= - 1'])
        ...
        could_be_zero_indicators = [
            ...
            used_in_gtenegativeone_check, is_offset_or_position
        ]
        could_be_zero_score = sum(could_be_zero_indicators) / max(len(could_be_zero_indicators), 1)
        
        # Add "could be zero" features (focus on >= -1 patterns)
        features.extend([
            ...
            float(used_in_gtenegativeone_check) * 2.0,
