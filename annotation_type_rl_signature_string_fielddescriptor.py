#!/usr/bin/env python3
"""
Annotation Type Reinforcement Learning Training Script for @FieldDescriptor
Uses binary RL models to train a separate model for predicting @FieldDescriptor annotation placement
...
class AnnotationTypeTrainer:
    """Trainer for @FieldDescriptor annotation type using Signature String Checker features"""
    
    def __init__(self, annotation_type='@FieldDescriptor', base_model_type='gcn', learning_rate=0.001, device='cuda'):
    ...
    def _init_annotation_model(self):
        """Initialize model for @FieldDescriptor annotation type prediction"""
    ...
    def extract_annotation_features(self, cfg_data, binary_predictions):
        ...
        for i, node in enumerate(nodes):
            ...
            # Determine if this node should have @FieldDescriptor annotation
    ...
    def _should_have_annotation_type(self, node):
        """Determine if node should have @FieldDescriptor annotation"""
        label = node.get('label', '').lower()
        
        # @FieldDescriptor: for strings in JVM field descriptor format (e.g., "Ljava/lang/String;")
        # Indicators:
        # - Starts with 'L' and ends with ';'
        # - Has slashes (internal format)
        # - Used in method descriptors
        # - Used in type conversion/reflection
        
        is_field_descriptor_format = label.startswith('l') and label.endswith(';')
        has_slashes = '/' in label
        is_method_descriptor = '(' in label and ')' in label
        has_primitive_type = any(pt in label for pt in ['i', 'j', 'd', 'f', 'z', 'b', 'c', 's'])
        
        return is_field_descriptor_format or (has_slashes and is_method_descriptor) or has_primitive_type
...
def main():
    parser = argparse.ArgumentParser(description=f'Training for @FieldDescriptor annotation type')
    ...
    trainer = AnnotationTypeTrainer(
        annotation_type='@FieldDescriptor',
    ...
    logger.info("@FieldDescriptor annotation type training completed successfully")
