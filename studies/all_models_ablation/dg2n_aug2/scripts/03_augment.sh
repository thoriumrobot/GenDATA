...
shared_aug = Path("/home/ubuntu/GenDATA/studies/all_models_ablation/shared_aug_slices_l2")
...
if not shared_aug.exists() or count_java(shared_aug)==0:
  ...
  print('GENERATED_SHARED_AUG_L2:' + str(count_java(shared_aug)))
else:
  print('USING_EXISTING_SHARED_AUG_L2 (' + str(count_java(shared_aug)) + ' files)')
PY
