...
This guide provides comprehensive instructions for running evaluations with the GenDATA auto-training system, which ensures all predictions are generated by trained machine learning models using graph-based CFG inputs.
...
- **Uses Enhanced Causal models by default** (32-dimensional features + graph embeddings)
...
- **✅ Enhanced Causal models** used by default (graph-augmented)
...
### **2. Large-Scale Evaluation (Graph-Based)**
```bash
# Run full prediction (CFG Builder + models only; graph inputs end-to-end)
...
- **Features**: 32-dimensional feature analysis (augmented with graph embeddings)
