...
class AnnotationTypeTrainer:
    ...
    def __init__(self, annotation_type='@Positive', base_model_type='gcn', learning_rate=0.001, device='cuda', checker_type=None):
    ...
    def _init_annotation_model(self):
        ...
        if CHECKER_MODULES_AVAILABLE and self.checker_type is not None:
            # Determine input dimension (base features + pattern features)
            base_input_dim = 14  # Standard feature dimension
    ...
    def extract_annotation_features(self, cfg_data, binary_predictions):
        ...
        for i, node in enumerate(nodes):
            ...
            # Only append graph embeddings for models that expect the extended feature vector.
            # Graph-based models (gcn/hgt/gcsn) are initialized with input_dim=14 and will
            # fail if we tack on the 256-dim embedding, so we skip embedding for them.
    ...
    def _extract_annotation_type_features(self, node, cfg_data):
        ...
        # "Could be zero" detection patterns (inverse signal for @Positive)
        ...
        # Add "could be zero" features (inverse signal for @Positive - low values indicate @Positive)
        features.extend([
            ...
            float(could_be_zero_score) * 3.0,  # Aggregated score
        ])
