...
It provides both enhanced and simple transformations that increase code variety 
while preserving semantics and being resistant to slicer pruning.

JDT-Based Transformations:
- Enhanced: Complex transformations for advanced augmentation
- Simple: Basic transformations for reliable augmentation
"""
...
HEADER_COMMENT = """
/*
 * CFWR semantic augmentation: applied semantic-preserving transformations using JDT AST parsing.
 */
""".strip()

class SemanticTransformer:
    """Semantic transformer using JDT-based transformations with both enhanced and simple modes."""
    ...
    def __init__(self, seed: int = 42, disabled_transformations: List[str] = None, jdt_jar_path: Optional[str] = None):
        ...
        try:
            ...
            logger.info("Initialized SemanticTransformer with JDT-based transformations")
        except Exception as e:
            logger.error(f"Failed to initialize JDT transformer: {e}")
            raise RuntimeError(f"JDT transformer initialization failed: {e}")
    
    def transform_file(self, java_path: str, variant_idx: int, mode: str = 'enhanced') -> str:
        """Apply semantic transformations to a Java file using JDT."""
        try:
            with open(java_path, 'r') as f:
                original_code = f.read()
            
            # Get available transformations for the specified mode
            available_transformations = self.jdt_transformer.get_available_transformations(mode)
            ...
            if not enabled_transformations:
                logger.warning(f"No transformations available for mode '{mode}' after filtering disabled ones")
            ...
            logger.info(f"Applying {mode} transformations: {selected_transformations}")
            ...
            if transformed_code != original_code:
                transformed_code = self._add_header_comment(transformed_code, selected_transformations, mode)
    ...
    def _add_header_comment(self, code: str, transformations: List[str], mode: str) -> str:
        """Add header comment with applied transformations."""
        transformation_list = ", ".join(transformations)
        header = f"{HEADER_COMMENT}\n// Applied {mode} transformations: {transformation_list}\n\n"
        return header + code
    
    def transform_directory(self, input_dir: str, output_dir: str, num_variants: int = 3, mode: str = 'enhanced') -> Dict[str, Any]:
        ...
        logger.info(f"Found {len(java_files)} Java files to process in {mode} mode")
        ...
        for java_file in java_files:
            try:
                ...
                for variant_idx in range(num_variants):
                    transformed_code = self.transform_file(str(java_file), variant_idx, mode)
                    
                    # Create output path
                    variant_name = f"{java_file.stem}_{mode}_variant_{variant_idx}{java_file.suffix}"
                    ...
                    logger.info(f"Created {mode} variant: {output_file}")
    ...
    def get_transformation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'available_enhanced_transformations': self.jdt_transformer.get_available_transformations('enhanced'),
            'available_simple_transformations': self.jdt_transformer.get_available_transformations('simple'),
            'disabled_transformations': self.disabled_transformations
        }

def main():
    """Main function for command-line usage."""
    parser = argparse.ArgumentParser(description='Semantic augmentation using JDT')
    ...
    parser.add_argument('--mode', choices=['enhanced', 'simple'], default='enhanced', 
                       help='Transformation mode (default: enhanced)')
    ...
    try:
        ...
        if os.path.isfile(args.input):
            # Single file
            transformed_code = transformer.transform_file(args.input, 0, args.mode)
            ...
            # Directory
            results = transformer.transform_directory(args.input, args.output, args.variants, args.mode)
            
            print(f"Processing complete:")
            print(f"  Files processed: {results['files_processed']}")
            print(f"  Variants created: {results['variants_created']}")
            print(f"  Mode used: {results['mode_used']}")
        ...
        print(f"  Available enhanced transformations: {len(stats['available_enhanced_transformations'])}")
        print(f"  Available simple transformations: {len(stats['available_simple_transformations'])}")
        ...
        logger.error(f"Semantic transformation failed: {e}")
        return 1
